  ...
  impl WidgetImpl for WaveformView {
    ...
    fn snapshot(&self, snapshot: &gtk::Snapshot) {
      ...
      if let Some(ref peaks) = *self.peaks.borrow() {
        ...
        let mut cursor_pos: [f64; 2] = [position * waveform_width, position * waveform_width];
...
impl WaveformView {
  ...
  fn setup_gesture(&self) {
    ...
    drag_gesture.connect_drag_begin(clone!(@strong self as this => move |gesture, start_x, _| {
        if !this.has_focus() {
            this.grab_focus();
        }
    ...
    drag_gesture.connect_drag_update(clone!(@strong self as this => move |gesture, offset_x, _| {
        if !this.has_focus() {
            this.grab_focus();
        }
        gesture.set_state(gtk::EventSequenceState::Claimed);
    ...
    key_controller.connect_key_released(clone!(@strong self as this => move |_, keyval, _, _| {
